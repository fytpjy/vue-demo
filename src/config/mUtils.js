 ...
 *  存储用户信息
 */
export const userInfoSave = (data) =>{
    wxlogin().then(res => {
        if(res.code==200){
            setStore("user_id",res.user_id)
            setStore("user_token",res.user_token)
            setStore("session_id",res.session_id)
        }else{
            Vue.$toast(res.msg)
        }
    })
...
export const cnzzCount = (data) => {
    if(data){
        ...
        countHeadCode.appendChild(document.createTextNode("var _czc = _czc || [];_czc.push(['_setAccount', "+data+"])"))
        document.getElementsByTagName("title")[0].before(countHeadCode);

        var countFootCode = document.createElement("script");
        countFootCode.src = "https://s4.cnzz.com/z_stat.php?id="+data+"&web_id="+data+"";
        document.getElementsByTagName("body")[0].append(countFootCode);
    }
}
...
export const wxConfig = (options) => {
    var wxCode = document.createElement("script");
    wxCode.setAttribute("type","text/javascript");
    wxCode.setAttribute('src',wxjsurl);
    document.getElementsByTagName("head")[0].appendChild(wxCode);
    if(options){
        ...
        wxCode.onerror = function(){
            console.error("微信加载失败，请刷新重试")
        }
    }
}
